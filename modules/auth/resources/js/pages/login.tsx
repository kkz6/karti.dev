...
import { Fingerprint, LoaderCircle, Shield } from 'lucide-react';
...
export default function Login({ status, canResetPassword }: LoginProps) {
    ...
    const { isLoading: passkeyLoading, error: passkeyError, authenticateUser } = usePasskeyAuth();
    ...
    return (
        <AuthLayout title="Log in to your account" description="Enter your email and password below to log in">
            ...
            <AnimatePresence mode="wait">
                {!showPasswordField ? (
                    ...
                    >
                        ...
                        <Button type="submit" className="w-full" disabled={loading || !data.email}>
                            {loading ? (
                                <>
                            ...
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                                ...
                                <div className="relative my-6">
                                    ...
                                    <div className="relative flex justify-center text-xs uppercase">
                                        <span className="bg-background text-muted-foreground px-2">Or use password</span>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                        ...
                        <form onSubmit={handlePasswordSubmit} className="space-y-4">
                            <div className="grid gap-2">
                                ...
                                <Input
                                    ...
                                    aria-invalid={Boolean(errors.password || errors.email)}
                                    aria-describedby={errors.password || errors.email ? 'password-error' : undefined}
                                />
                                <InputError id="password-error" message={errors.password || errors.email} />
                            </div>
            ...
            {passkeyError && <div className="mt-4 text-center text-sm font-medium text-red-600">{passkeyError}</div>}
