...
export default function EmptyState({ title = '', message = '', actions = [], icon = true, iconResolver, dataAttributes = null }) {
    function handleAction(url) {
        url.modal ? visitModal(url) : visitUrl(url)
    }
    ...
    return (
        ...
        >
            ...
            <div className="it-empty-state-actions mt-4 flex flex-wrap items-center justify-center gap-2">
                {actions
                    .filter((action) => !action.url.hidden)
                    .map((action, key) => (
                        <Button
                            key={key}
                            as={action.url.asDownload ? 'a' : 'button'}
                            className="it-empty-state-action-button"
